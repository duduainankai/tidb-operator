...
func (s *tiflashScaler) ScaleIn(meta metav1.Object, oldSet *apps.StatefulSet, newSet *apps.StatefulSet) error {
	...
	if tc.Spec.TiFlash.ScaleInParallelism != nil {
		scaleInParallelism = int(*tc.Spec.TiFlash.ScaleInParallelism)
	}
	...
	scaleInOne := func(ordinal int32) (bool, error) {
		...
		if err != nil {
			return false, fmt.Errorf("tiflashScaler.ScaleIn: failed to get pods %s for cluster %s/%s, error: %s", podName, ns, tcName, err)
		}
		...
		for _, store := range tc.Status.TiFlash.Stores {
			if store.PodName == podName {
				...
				return false, controller.RequeueErrorf("TiFlash %s/%s store %d is still in cluster, state: %s", ns, podName, id, state)
			}
		}
		...
		if !podutil.IsPodReady(pod) {
			...
			if time.Now().Before(safeTimeDeadline) {
				...
				return false, fmt.Errorf("TiFlash %s/%s is not ready, wait for some resync periods to synced its status", ns, podName)
			}
		...
		return false, fmt.Errorf("tiflash %s/%s no store found in cluster", ns, podName)
	}
